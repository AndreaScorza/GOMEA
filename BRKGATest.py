            ...
            if markedGoods[y] == 1:
                ...
                if discourage:
                    if x[1] > 0.5:
                        x[1] = 1 - x[1]
        if not flag:
            solution.append(bids[x[0]])
            fitness += bidsValue[x[0]]
            for y in bids[x[0]]:
                markedGoods[y] = 1
    element.sort(key=lambda x: x[0])
    return element, round(fitness, 5), solution
...
def BRKGAchromo(populationSize, problem):
    ...
    while fitNotIncrease < 200 and generationCount < 5000:
        ...
        if generationCount % 10 == 0:
            ...
            for x in population:
                el, fi, so = (decoder(x, False, goods, bids, bidsValue))
                elem.append(el)
                fit.append(fi)
                sol.append(so)
            index = fit.index(max(fit))
            print(fit[index], sol[index], len(sol[index]))
...
bestFitness, storedPop, lastPopulation, totalTime, foundAtGen, totFitEval, values = BRKGAchromo(500, "L6.txt")
goods = values[0]
bidsValue = values[3]
bids = values[4]
...
index = fit.index(max(fit))
print(fit[index], sol[index], "len: ", len(sol[index]))
